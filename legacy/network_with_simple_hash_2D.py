...
class ComplexINRModel2D(nn.Module):
    ...
    def forward(self):
        ...
        if self.share_spatial_features:
            ...
            # Upsample to full resolution (添加 batch 和 channel 维度进行上采样，然后去掉)
            object_real_part = self.upsample(object_real_part.unsqueeze(0).unsqueeze(0)).squeeze(0).squeeze(0)  # [W, H]
            object_imag_part = self.upsample(object_imag_part.unsqueeze(0).unsqueeze(0)).squeeze(0).squeeze(0)  # [W, H]
