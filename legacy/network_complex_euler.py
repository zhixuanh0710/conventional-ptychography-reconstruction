...
class ComplexGaborLayer2D(nn.Module):
    '''
        ...
        参数:
            ...
            out_features: 输出特征维度
            bias: 是否使用偏置
            is_first: 是否为第一层（影响数据类型）
            omega0: 复数指数的频率参数
            trainable: omega是否可训练
    '''
    
    def __init__(self, in_features, out_features, bias=True,
                 is_first=False, omega0=100.0, trainable=False):
        super().__init__()
        self.omega_0 = omega0
    ...
    def forward(self, input):
        ...
        return torch.exp(1j*self.omega_0*lin)
...
class ComplexMLP(nn.Module):
    ...
    def __init__(self, in_features, hidden_features, 
        ...
        self.net.append(ComplexGaborLayer2D(in_features,
                                            ...
                                            is_first=True,
                                            trainable=trainable_omega0))

        # 隐藏层
        for i in range(hidden_layers):
            self.net.append(ComplexGaborLayer2D(hidden_features,
                                                hidden_features, 
...
class ComplexINRModel2D(nn.Module):
    ...
    def __init__(
        ...
        self.object_complex_mlp = ComplexMLP(
        ...
        # Probe MLP (使用复数指数激活) - 可选
        if update_probe:
            self.probe_complex_mlp = ComplexMLP(
                in_features=self.num_features,
                hidden_features=hidden_features,
                hidden_layers=hidden_layers,
                out_features=1,
    ...
    def init_residual_params(self, object_initial, probe_initial):
        ...
        - k 是实数类型的可学习参数，初始化为 1e-8
        
        训练策略（当有 initial 时）：
        - 训练初期: k≈1e-8，output ≈ initial_value（主要依赖初始值）
        ...
        if object_initial is not None:
            ...
            # 创建可学习的缩放因子 k (实数)
            self.k_object = nn.Parameter(torch.tensor(1e-8, dtype=torch.float32))
        ...
        if self.update_probe:
            if probe_initial is not None:
                ...
                # 创建可学习的缩放因子 k (实数)
                self.k_probe = nn.Parameter(torch.tensor(1e-8, dtype=torch.float32))
    ...
    def _get_complex_output(self, model, coords):
        """
        从 ComplexMLP 模型获取完整的复数输出
        forward 默认返回 output.real，这里我们直接调用网络获取复数
        """
...
if __name__ == "__main__":
    ...
    assert not torch.is_complex(model_res.k_object), "k_object 应该是实数"
    assert abs(model_res.k_object.item() - 1e-8) < 1e-10, "k_object 应该初始化为1e-8"
    if model_res.has_probe_residual:
        ...
        assert not torch.is_complex(model_res.k_probe), "k_probe 应该是实数"
        assert abs(model_res.k_probe.item() - 1e-8) < 1e-10, "k_probe 应该初始化为1e-8"
    ...
    print("✓ 使用复数指数激活函数 exp(iω·x)!")
    print("✓ Conventional ptychography 模式: Object + Probe")
    print("="*60)
